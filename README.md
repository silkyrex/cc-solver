...
7. ~~**`graduated_date` is written and never read.**~~ **Handled 2026-09-09.** `doors.exit_state` now takes `graduated_date` (keyword-only) and `cli.position_monitor` passes the held row's field through. A stored date settles graduation outright — no scan runs, so no bar window can demote the position, which is the whole rule. A **derived** graduation now reports the date it happened (`graduated_date` + `graduated_source: "derived"`) and `position_monitor` returns a `graduated_payloads` list for EOD to write into the Positions DB "Graduated on" column; a bare `graduated: true` would have left the same bug live for the next 130 days. Refusals are loud, never silent: a date **before** entry, or **after** the last settled bar (only a settled close may graduate, same invariant the provisional bar has), is rejected with a reason in `graduated_date_error`, falls back to the bar scan, and pushes. **Residuals:** the write-back half is repo-side only — the EOD prompt has to map `graduated_payloads` before "Graduated on" is actually filled, and until it is, every derived graduation is re-derived daily. A stored date on a `21ema` entry is ignored rather than flagged (graduation is moot there; it cannot move the door in either direction). And the settled-bar check compares against the last bar the harness supplied, so a stale bar feed can refuse a legitimate same-week date — loudly, and on the tight door.
