...
| `cli.py` | `take_action`, `confirm_pass` (staged flips + pending exits), `position_monitor`, `fast_discovery`, `eow`, `miss_audit`, `calendar` | anything not in `--inputs` |
...
- **The 12:35 PM slot is where a pending exit is still actionable.** `confirm_pass` returns `pending_exits` alongside `flips`: held names whose live price puts them on track to trip their door at the close. `position_monitor` at 1:10 PM confirms on settled closes. Nothing in `pending_exits` is a confirmed exit.
