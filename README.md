...
7. ~~**`graduated_date` is written and never read.**~~ **Handled 2026-09-09.** `doors.exit_state` now takes `graduated_date` (keyword-only) and `cli.position_monitor` passes the held row's field through. A stored date settles graduation outright — no scan runs, so no bar window can demote the position, which is the whole rule. A **derived** graduation now reports the date it happened (`graduated_date` + `graduated_source: "derived"`) and `position_monitor` returns a `graduated_payloads` list for EOD to write into the Positions DB "Graduated on" column; a bare `graduated: true` would have left the same bug live for the next 130 days. Refusals are loud, never silent: a date **before** entry, or **after** the last settled bar (only a settled close may graduate, same invariant the provisional bar has), is rejected with a reason in `graduated_date_error`, falls back to the bar scan, and pushes. **Residuals:** the write-back is a prompt, not the repo, and it is already most of the way there — the live EOD prompt (step 5b) writes "Graduated on" the run it first sees `graduated=true`, but with **today's** date rather than the session it happened on. Those differ whenever EOD skips a day (holiday, `Partial`, a close that never settled) or the row was backfilled, and the misdating is permanent. Pointing that clause at `graduated_payloads` is a one-line prompt change, written out in the handoff. A stored date on a `21ema` entry is ignored rather than flagged (graduation is moot there; it cannot move the door in either direction). And the settled-bar check compares against the last bar the harness supplied, so a stale bar feed can refuse a legitimate same-week date — loudly, and on the tight door.
