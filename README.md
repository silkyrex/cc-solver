...
**Where the live fleet actually gets bars.** Every Fleet v2 task pulls them from Robinhood `get_equity_historicals` (interval `day`, bounds `regular`, `adjustment_type` `split`, `start_time` 130 calendar days ago), not IBKR `get_price_history`. That matters because of known gap #6 below.
...
6. **Nothing filters interpolated bars.** Robinhood back-fills a requested date range with SYNTHETIC bars carrying an `interpolated` flag (DEC-006 data note: SKHY 486/527 synthetic, SPCX 468, DRAM 419, CRCL 212). The bar contract here has no such field, no live task prompt strips them, and a fabricated flat bar is indistinguishable from a real one once it reaches `indicators`. For a name younger than the 130-day pull its whole 21 EMA can be fabricated. The handling DEC-006 specifies: keep the flag, and strip the LEADING run and the TRAILING run separately — today's forming bar is also flagged, so "drop everything up to the last flagged bar" deletes whole histories.
